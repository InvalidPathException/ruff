...
then that particular task is considered invalid.
*/
...
#[derive(Debug, clap::Parser)]
struct AllCommand {
    /// The mean recriporcal rank threshold that the evaluation must
    ...
    fn matches_task(&self, task: &Task) -> bool {
        self.task_name == task.name
            && self
                .file_name
                .as_ref()
                .map_or(true, |name| name == task.cursor_name())
            && self.index.map_or(true, |index| index == task.cursor.index)
    }
}
...
fn main() -> anyhow::Result<ExitCode> {
    ...
    match args.command {
        Command::ShowOne(ref cmd) => {
            ...
            let tasks = source.to_tasks(&tmp_eval_dir)?;
        ...
        Command::All(AllCommand { threshold, tasks }) => {
            ...
            for source in &sources {
                for task in source.to_tasks(&tmp_eval_dir)? {
                    let completions = task.completions()?;
                    let rank = task.rank(&completions)?;
                    precision_sum += rank.map(|rank| 1.0 / (rank as f64)).unwrap_or(0.0);
                    if let Some(ref mut wtr) = results_wtr {
                        wtr.serialize((
                            &task.name,
                            &task.cursor.path.as_str(),
                            task.cursor.index,
                            rank,
                        ))?;
                    }
            ...
            let mrr = precision_sum / (sources.len() as f64);
            if let Some(ref mut wtr) = results_wtr {
                wtr.flush()?;
            }

            let mut out = std::io::stdout().lock();
            writeln!(out, "mean reciprocal rank: {mrr}")?;
            if mrr < threshold {
                writeln!(out, "MRR does not exceed threshold of {threshold}")?;
                Ok(ExitCode::FAILURE)
            } else {
                writeln!(out, "MRR exceeds threshold of {threshold}")?;
                Ok(ExitCode::SUCCESS)
            }
        }
    }
...
impl CompletionAnswer {
    ...
    fn matches(&self, completion: &Completion) -> bool {
        self.symbol == completion.name.as_str()
            && self.module.as_deref() == completion.module_name.map(|name| name.as_str())
    }
}
